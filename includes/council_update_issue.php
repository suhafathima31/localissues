<?php
// Council update issue status API endpoint
...
try {
    ...
    if ($result && $stmt->rowCount() > 0) {
        ...
        $log_stmt = $pdo->prepare("
            INSERT INTO issue_updates_log (issue_id, council_user_id, old_status, new_status, updated_at) 
            VALUES (?, ?, ?, ?, NOW())
        ");
        // Note: You would need to create this table if you want audit logging
        
        http_response_code(200);
        echo json_response(true, 'Issue status updated successfully by municipal council', [
            'issue' => $updated_issue
        ]);
    } else {
    ...
    log_error('Database error in council_update_issue.php: ' . $e->getMessage());
    http_response_code(500);
    echo json_response(false, 'Database error occurred');
} catch (Exception $e) {
    log_error('General error in council_update_issue.php: ' . $e->getMessage());
    http_response_code(500);
